...
class GreenFuncNph : public Diagram {
    ...
    inline double electronDispersion(double kx, double ky, double kz){return (std::pow(kx,2) + std::pow(ky,2) + std::pow(kz,2))/(2*_el_eff_mass);};
    // phonon dispersion relation inline
    inline double phononDispersion(){return _ph_dispersion;};
